...
impl TryFrom<(u32, Vec<u8>)> for Receiver {
    type Error = ParseError;

    fn try_from((typecode, addr): (u32, Vec<u8>)) -> Result<Self, Self::Error> {
...
impl private::Receiver for Receiver {
...
mod tests {
    ...
    proptest! {
        ...
        fn ua_roundtrip(
            ...
            let decoded = Address::from_bytes(hrp.as_str(), &bytes[..]);
    ...
    fn padding() {
        ...
        assert_eq!(
            Address::from_bytes(Address::MAINNET, &invalid_padding[..]),
        ...
        assert_eq!(
            Address::from_bytes(Address::MAINNET, &truncated_padding[..]),
            Err(ParseError::InvalidEncoding(
                "Invalid padding bytes".to_owned()
            ))
        );
    ...
    fn truncated() {
        ...
        assert_matches!(
            Address::from_bytes(Address::MAINNET, &truncated_sapling_data[..]),
        ...
        assert_matches!(
            Address::from_bytes(Address::MAINNET, &truncated_after_sapling_typecode[..]),
            Err(ParseError::InvalidEncoding(_))
        );
    }
    ...
    fn duplicate_typecode() {
        ...
        assert_eq!(
            Address::from_bytes(Address::MAINNET, &encoded[..]),
